		    ...
		    for(int ip =0; ip<code.length;ip++)
		     {
       	    	...
       	    	if(code[ip].contains("load") && !whileFlag){
		    		...
		    		String[] params = null;
		    		Runtime2.parseFunctions(fBody,params);
		    	}
...
public static void parseFunctions(String[] body, String[] params){
	...
	for(int ip=0; ip<body.length; ip++){
		...
		if(body[ip].contains("load")){
			...
			//Store the symbol table in activation stack
	    	...
	    	if(values.length>2){
	    			int j=0;
	    			System.out.println("dsdsdsdff" + values[2]);
	    			temp = values[2];
...
//	    			}
			...
			if(funcparam.length>2){
    			...
    			if(temp.matches("[0-9]+"))
    				...
    				localSt.push(Integer.parseInt(localSymbolTable.get(temp)));
//    			for(int i =3;i<values.length;i=i+2){
//    				String par = values[i];
//    				localSymbolTable.put(par, temp);
//    			}
    		}
			Runtime2.parseFunctions(funcallbody,params);
		}
    		...
    		String[] whileLoopBody = whileBody.toArray(new String[0]);
   			Runtime2.parseWhileLoop(whileLoopBody);
